from app.database.elastic import es_client
...
def index_menu(menu):
    ...
    menu_id = str(getattr(menu, "id", None) or getattr(menu, "_id", None))

    #Creamos el documento que se va a guardar en elastic 
    document = {
        "id": menu_id,
        "nombre": getattr(menu, "nombre", ""),
        "categoria": getattr(menu, "categoria", ""),
        "descripcion": getattr(menu, "descripcion", None) or "Producto sin descripción",
        "precio": float(getattr(menu, "precio", 0)),
        "disponible": getattr(menu, "disponible", True),
        "tiempo_preparacion": int(getattr(menu, "tiempo_preparacion", 0)),
        "restaurante_id": str(getattr(menu, "restaurante_id", ""))
    }
